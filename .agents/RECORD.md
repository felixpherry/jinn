...
- (ui) All TUI clipboard copies route through a shared native clipboard backend supporting Wayland data-control alongside existing platform backends.
- (ui) TUI clipboard write failures are reported through a visible status hint and the application log.
